...
function renderButtons(){
  ...
  for(const r of RANKS){
    ...
    b.title=`ลบ ${DISPLAY[r]} 1 ใบ (เหลือ ${baseCounts[r]} ใบ)`;
    b.disabled=baseCounts[r]<=0;
    b.onclick=()=>{ if(baseCounts[r]>0){ baseCounts[r]-=1; removed.push(r); renderAll(); } };
    rankButtonsDiv.appendChild(b);
  }
}
function renderRemoved(){
  removedListDiv.innerHTML="";
  if(removed.length===0){ const s=document.createElement("span"); s.textContent="— ไม่มี —"; s.style.color="#64748b"; removedListDiv.appendChild(s); return; }
  for(const r of removed){ const tag=document.createElement("span"); tag.className="badge"; tag.textContent=DISPLAY[r]; removedListDiv.appendChild(tag); }
}
...
btnUndo.onclick=()=>{ const r=removed.pop(); if(r) baseCounts[r]+=1; renderAll(); };
btnReset.onclick=()=>{ baseCounts=makeCounts(); removed=[]; renderAll(); };
btnPreset1.onclick=()=>{ for(const r of [11,11,13,13,9]){ if(baseCounts[r]>0){ baseCounts[r]-=1; removed.push(r); } } renderAll(); };
...
function renderRecommendation(){
  ...
  let rec="รอ/ข้ามตานี้", sub="ยังไม่มีทางเลือกที่ EV เป็นบวก";
  let cls="neutral";
